...
public:
	...
	class ShapeElem //Nested Class
	{
		public:
			...
			ShapeElem(){}
			void setVPointer(void * vPtr){vPointer = vPtr;} //Pointer atama 
			void setType(Shapes s){type_shape = s;} //Tip atama
			Shapes getType()const {return type_shape;} //Tip return etme
			void * getPtr()const {return vPointer;} //Pointer return etme
			double ShapeElem_area()const;
			double ShapeElem_PL()const;

		private:
			void* vPointer; //Void pointer
			Shapes type_shape; // Tip objesi
	};
	...
	friend ostream& operator<< (ostream &file, const ComposedShape & compos);

private:

	vector <ShapeElem> vec_ShapeElem; //ShapeElem vectoru
